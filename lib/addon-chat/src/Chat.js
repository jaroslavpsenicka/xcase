...
const Chat = ({ caseId, serviceUrl }) => {  
  ...
  const handleSubmit = (message) => {
    ...
    Axios.post(serviceUrl + '/api/assistant/' + session.session_id + '/message', payload)
  ...
  useEffect(() => {
    Axios.post(serviceUrl + '/api/assistant')
      .then(response => {
        setSession({ loading: false, session_id: response.data.session_id });
        Axios.post(serviceUrl + '/api/assistant/' + response.data.session_id + '/message')
          .then(response => handleResponse(response))
          .catch(err => handleError(err));
        })
